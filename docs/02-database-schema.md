...
- `id` (String, Primary Key) - Account number from bank (e.g., "LT000000000000000004")
...
- Primary key on `id`
- Unique constraint on account identifier
...
- Account IDs come from CSV data (bank account numbers)
- Accounts are created automatically during first import
- No sensitive information like full account numbers (use masked/hashed if needed)
...
- `accountId` (String, Foreign Key → accounts.id) - Which account this transaction belongs to
