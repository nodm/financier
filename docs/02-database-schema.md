...
**Fields** (To Be Determined during implementation):

Suggested fields:

- `id` (String, Primary Key) - Internal UUID
- `accountNumberHash` (String, Unique) - SHA-256 hash of the full bank account number (for lookups)
- `mask` (String) - Last 4 digits of account number (e.g., "1234")
- `name` (String, Optional) - User-friendly account name
- `type` (String) - Account type (e.g., "checking", "savings", "credit")
- `currency` (String) - Primary currency (ISO 4217 code)
- `bankCode` (String) - Which bank this account belongs to
- `createdAt` (DateTime) - When account was first seen
- `updatedAt` (DateTime) - Last time account was updated

**Constraints**:

- Primary key on `id`
- Unique constraint on `accountNumberHash`

**Indexes**:

- Primary key index (automatic)
- Index on `bankCode` for filtering by bank
- Index on `accountNumberHash` for lookups during import

**Notes**:

- **Privacy**: Full account numbers are NOT stored in the database.
- `accountNumberHash` is used to link CSV imports to existing accounts.
- `mask` is used for UI display to identify accounts.
- Accounts are created automatically during first import.
...
- `accountId` (String, Foreign Key → accounts.id) - Internal UUID of the account
- `date` (DateTime) - Transaction date (from bank statement)
- `amount` (Decimal) - Transaction amount (negative for debits, positive for credits)
...
- externalId may not exist for all banks (use NULL)
...
One account has many transactions. Each transaction belongs to exactly one account.

## Prisma Schema Template

**Location**: `packages/db/prisma/schema.prisma`

**Template** (fields to be finalized during implementation):
...
- Exact field names and types to be determined during implementation
